...
- All processing/publishing runs are tied to the signed-in user and logged to the `history` table. Session tokens are hashed at rest; they auto-issue on both Google and local logins so returning users are signed in automatically (the token is stored in the URL query params + Streamlit session state).
...
- **Cloud deployment**: On AWS/other clouds, run `streamlit run app.py` behind TLS, persist `logs/app.db` on durable storage, and set env vars for Google/TikTok OAuth secrets. Add network egress controls if processing sensitive media.
