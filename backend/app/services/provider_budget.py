...
class ProviderBudgetStore:
    ...
    def reserve(
        ...
        *,
        ...
        with self.db.session() as session:
            return self.reserve_in_session(
                session,
    ...
    def reserve_in_session(
        ...
        *,
        ...
        """Reserve provider money inside the caller's atomic transaction."""
        ...
        for key, scope, period_start in (
            ...
            session.execute(
                ...
                .on_conflict_do_nothing(
                    index_elements=[DbProviderBudgetWindow.key],
                )
            )
        ...
        windows = list(
            session.scalars(
                ...
                .where(
                    DbProviderBudgetWindow.key.in_(
    ...
    def release_in_session(
        ...
        """Release a reservation inside the caller's atomic transaction."""
        self._settle_in_session(
            session,
            idempotency_key=idempotency_key,
            actual_usd=None,
        )
    ...
    def finalize(self, idempotency_key: str, *, actual_usd: float) -> None:
        ...
        if not math.isfinite(actual) or actual < 0:
            raise ValueError(
                "Provider actual cost must be finite and non-negative",
            )
        self._settle(idempotency_key=idempotency_key, actual_usd=actual)

    def finalize_in_session(
        self,
        session: Session,
        idempotency_key: str,
        *,
        ...
        """Finalize provider spend inside the caller's atomic transaction."""
        actual = float(actual_usd)
        if not math.isfinite(actual) or actual < 0:
            raise ValueError(
                "Provider actual cost must be finite and non-negative",
    ...
    def _settle_in_session(
        ...
        *,
        ...
        window_keys = [
            reservation.daily_window_key,
            reservation.monthly_window_key,
        ]
        windows = list(
            session.scalars(
                select(DbProviderBudgetWindow)
        ...
        for window in windows:
            window.reserved_usd = max(
                0.0,
                float(window.reserved_usd) - estimate,
            )
