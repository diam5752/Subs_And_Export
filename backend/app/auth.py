...
def _get_secret(key: str) -> str | None:
    """Safely read a secret from .streamlit/secrets.toml if available."""
    try:
        # Try reading from .streamlit/secrets.toml directly
        import tomllib
        from pathlib import Path
        
        # Look for secrets.toml in the project root's .streamlit directory
        # We assume the backend is running from the project root or we can find it relative to this file
        # file is at backend/app/auth.py
        # project root is ../.. from this file's parent directory
        
        secrets_path = Path(".streamlit/secrets.toml")
        if not secrets_path.exists():
            # Try going up from this file's location
            secrets_path = Path(__file__).parent.parent.parent / ".streamlit" / "secrets.toml"
            
        if secrets_path.exists():
            with open(secrets_path, "rb") as f:
                secrets = tomllib.load(f)
                return str(secrets.get(key)) if key in secrets else None
    except Exception:
        pass
...
def google_oauth_config() -> dict[str, str] | None:
    """
    Read Google OAuth configuration from environment or secrets.toml.
    ...
    redirect_uri = os.getenv("GOOGLE_REDIRECT_URI") or _get_secret("GOOGLE_REDIRECT_URI")
