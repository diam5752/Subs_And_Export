...
def test_auth_mutation_ratelimit(client, monkeypatch):
    ...
    for i in range(5):
        res = client.put("/auth/me", json={"name": f"Name{i}"}, headers=headers)
        assert res.status_code == 200, f"Request {i} failed: {res.text}"

    # 4. Next request should fail
    res_fail = client.put("/auth/me", json={"name": "Fail"}, headers=headers)
    ...
    res_fail_pwd = client.put(
        "/auth/password", json={"password": "NewPassword123!", "confirm_password": "NewPassword123!"}, headers=headers
    )
    assert res_fail_pwd.status_code == 429

    # 6. Verify delete account also blocked (shared quota for mutation)
    res_fail_del = client.delete("/auth/me", headers=headers)
