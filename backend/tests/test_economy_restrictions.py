...
def test_concurrent_jobs_limit_process(client, user_auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        JobStore, "count_active_jobs_for_user", lambda self, uid: config.settings.max_concurrent_jobs + 1
    )

    # Mock probe_media to avoid actual file processing (though it happens after check)
    monkeypatch.setattr(
        "backend.app.api.endpoints.videos.probe_media", lambda p: MediaProbe(duration_s=10.0, audio_codec="aac")
    )
