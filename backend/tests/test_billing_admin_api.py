    ...
    *,
    ...
    with db.session() as session:
        ...
        if status == "succeeded" and active:
            purchase.status = (
                "refunded"
                if amount_cents == purchase.amount_eur_cents
    ...
    *,
    ...
    while True:
        ...
        assert cursor is not None, (
            f"Pending {resource} review {identity_value} was not listed"
        )
        assert cursor not in seen_cursors
        seen_cursors.add(cursor)
...
def test_manual_refund_accounting_is_append_only_exactly_replayable_and_local(
    ...
    recorded = client.post(
        (
            f"/billing/admin/refunds/{reversal_id}/"
            "record-aade-adjustment"
        ),
        headers=headers,
        json=payload,
    )
    ...
    assert result["original_invoice_mark"] == (
        payload["original_document"]["mark"]
    )
    assert result["aade_document_type"] == "11.4"
    assert result["aade_mark"] == (
        payload["adjustment_document"]["mark"]
    )
    ...
    with db.session() as session:
        ...
        assert adjustment.document_snapshot[
            "automatic_stripe_refund_executed"
        ] is False
        assert adjustment.document_snapshot[
            "automatic_aade_adjustment_executed"
        ] is False
        assert adjustment.financial_retention_until >= (
            result["financial_retention_until"]
        )
        assert invoice.financial_retention_until >= (
            adjustment.financial_retention_until
        )
        assert purchase.financial_retention_until >= (
            adjustment.financial_retention_until
        )
        ...
        assert (
            session.scalar(
                select(DbBillingAdjustmentRecord).where(
                    DbBillingAdjustmentRecord.reversal_id
                    == reversal_id,
                )
            )
            is None
...
def test_accepted_withdrawal_requires_and_exposes_completed_manual_actions(
    ...
    no_evidence = client.post(
        ...
        json={
            ...
            "customer_explanation": (
                "Η αίτησή σας εγκρίθηκε μετά από ανθρώπινο έλεγχο."
            ),
            "final_manual_review_confirmed": True,
        },
    )
    assert no_evidence.status_code == 409
    assert no_evidence.json()["detail"] == (
        "Accepted withdrawal adjustment was not found"
    )
    ...
    assert pending_after["available_adjustments"] == [
        {
            ...
            "currency": "eur",
            "aade_document_type": (
                accounting_payload["adjustment_document"][
                    "document_type"
                ]
            ),
            "aade_series": (
                accounting_payload["adjustment_document"]["series"]
            ),
            "aade_aa": (
                accounting_payload["adjustment_document"]["aa"]
            ),
            "aade_mark": (
                accounting_payload["adjustment_document"]["mark"]
            ),
            "issued_at": (
                accounting_payload["adjustment_document"]["issued_at"]
            ),
        }
    ]

    explanation = (
        "Η αίτησή σας εγκρίθηκε μετά από ανθρώπινο έλεγχο και "
        "η επιστροφή ολοκληρώθηκε χειροκίνητα."
    )
    resolution_payload = {
        "decision": "accepted_refunded",
        "adjustment_id": adjustment_id,
    ...
    assert resolution["resolution_url"] == (
    ...
    conflict = client.post(
        ...
        json={
            **resolution_payload,
            "customer_explanation": (
                "Διαφορετική τελική αιτιολογία που δεν αποτελεί replay."
            ),
        },
    )
    assert conflict.status_code == 409
    ...
    assert purchase["withdrawal_resolution_url"] == (
        f"/billing/purchases/{purchase_id}/withdrawal-resolution"
    )
    ...
    assert document["manual_actions"]["stripe_refund_id"] == (
        stripe_refund_id
    )
    assert document["manual_actions"]["aade_adjustment_id"] == (
        adjustment_id
    )
...
def test_rejected_withdrawal_records_no_refund_or_aade_action(
    ...
    explanation = (
        "Η αίτηση εξετάστηκε χειροκίνητα και δεν πληροί τις "
        "προϋποθέσεις της υποχρεωτικής επιστροφής."
    )
    ...
    artifact = client.get(
        (
            f"/billing/purchases/{purchase_id}/"
