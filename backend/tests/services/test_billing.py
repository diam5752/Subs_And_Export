...
def test_legacy_webhook_hash_recovers_canceled_authorization_retry(
    ...
    # REGRESSION: legacy receipts hashed the complete delivery envelope. Stripe
    # updates pending_webhooks between retries, so the old hash must be upgraded
    # only when every other raw payload byte is unchanged.
    db, user_id, points, gateway, service = _service()
    checkout = service.create_checkout(
        user_id=user_id,
        customer_email=f"{user_id}@example.com",
