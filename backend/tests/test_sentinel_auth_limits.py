...
def test_login_long_username(client_with_mocks, mock_user_store):
    ...
    response = client_with_mocks.post("/auth/token", data={"username": long_email, "password": "password123"})
...
def test_login_long_password(client_with_mocks, mock_user_store):
    ...
    long_password = "a" * 129
    response = client_with_mocks.post("/auth/token", data={"username": "valid@example.com", "password": long_password})
    assert response.status_code == 400
...
def test_login_normal_length(client_with_mocks, mock_user_store, mock_session_store):
    ...
    response = client_with_mocks.post("/auth/token", data={"username": "user@example.com", "password": "password123"})
