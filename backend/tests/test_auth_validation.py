...
class TestEmailValidation:
    ...
    def test_invalid_emails_registration(self, client):
        ...
        for email in invalid_emails:
            ...
            response = client.post(
                "/auth/register", json={"email": email, "password": "ValidPassword123", "name": "Test User"}
            )
    ...
    def test_valid_emails_registration(self, client):
        ...
        valid_emails = ["valid.user@example.com", "user+tag@domain.co.uk", "123user@sub.domain.org"]
        ...
        for base_email in valid_emails:
            ...
            response = client.post(
                "/auth/register", json={"email": unique_email, "password": "ValidPassword123", "name": "Test User"}
            )
            # Should succeed (200)
            assert response.status_code == 200, (
                f"Email {email} should be accepted, got {response.status_code} {response.text}"
            )
