...
def test_validate_path_is_safe_nonexistent_inside(tmp_path):
    ...
    # Note: pathlib.resolve() behavior on non-existent files depends on strictness.
    # In Python < 3.10, resolve() might not resolve non-existent segments fully,
    # but validate_path_is_safe calls resolve().
    # However, if the path doesn't exist, we usually want to know if WRITING to it is safe.
