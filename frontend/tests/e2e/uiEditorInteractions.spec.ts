...
test("the active subtitle can be dragged and resized directly on the desktop preview", async ({
  page,
}) => {
  ...
  const overlay = page.getByTestId("subtitle-overlay");
  const moveHandle = page.getByRole("slider", {
  ...
  await expect(
    page.getByTestId("subtitle-direct-manipulation-hint"),
  ).toHaveCount(0);
  ...
  await expect
    .poll(async () => Number(await overlay.getAttribute("data-position")))
