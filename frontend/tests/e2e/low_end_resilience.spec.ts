...
test("a transient initial session failure exposes retry and then restores the user", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: el.sessionUnavailableTitle }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: el.sessionRetry }),
  ).toBeVisible();
...
test(
  "the completed editor stays within low-end mobile performance budgets with real media",
  {
    tag: "@performance",
  },
  async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      (
        window as typeof window & { __lowEndMetrics?: LowEndMetrics }
      ).__lowEndMetrics = metrics;
      try {
    ...
    try {
      for (
        let sampleIndex = 0;
        sampleIndex < LOW_END_SAMPLE_COUNT;
        sampleIndex += 1
      ) {
        ...
        measurements.push(
          await page.evaluate(
            ({ readyMs: sampleReadyMs, scrollFrames: frames }) => {
              const metrics = (
                window as typeof window & { __lowEndMetrics?: LowEndMetrics }
              ).__lowEndMetrics ?? { cls: 0, longTasks: [] };
              const video = document.querySelector("video");
              const header =
                document.querySelector<HTMLElement>(".studio-header");
              const sortedFrames = [...frames].sort((a, b) => a - b);
              const initialScriptDecodedBytes = performance
              ...
              return {
                ...
                backdropFilter: header
                  ? getComputedStyle(header).backdropFilter
                  : "",
                videoDuration: video?.duration ?? 0,
                videoWidth: video?.videoWidth ?? 0,
                p95FrameMs:
                  sortedFrames[Math.floor(sortedFrames.length * 0.95)] ?? 0,
                framesOver50Ms: sortedFrames.filter((duration) => duration > 50)
                  .length,
              };
            },
    ...
    const normalizedTbtSamples = measurements.map((sample) => ({
      ...
      normalizedTbt:
        sample.approxTbt *
        Math.min(1, LOW_END_REFERENCE_READY_MS / Math.max(1, sample.readyMs)),
    }));
    ...
    const sortedNormalizedTbt = normalizedTbtSamples
      ...
      .sort((a, b) => a - b);
    const medianRawTbt =
      sortedRawTbt[Math.floor(sortedRawTbt.length / 2)] ??
      Number.POSITIVE_INFINITY;
    const medianNormalizedTbt =
      sortedNormalizedTbt[Math.floor(sortedNormalizedTbt.length / 2)] ??
      Number.POSITIVE_INFINITY;
    ...
    for (const measurement of measurements) {
      ...
      expect(measurement.approxTbt).toBeLessThanOrEqual(
        LOW_END_RAW_TBT_CEILING_MS,
      );
      expect(measurement.initialScriptDecodedBytes).toBeGreaterThan(0);
      expect(measurement.initialScriptDecodedBytes).toBeLessThanOrEqual(
        LOW_END_INITIAL_SCRIPT_BUDGET_BYTES,
      );
    ...
    expect(medianNormalizedTbt).toBeLessThanOrEqual(
      LOW_END_NORMALIZED_TBT_BUDGET_MS,
    );
  },
);
