...
test.describe("Video Processing Flow", () => {
  test("mock processing accepts promotional credits without claiming a provider call", async ({
    page,
  }) => {
    ...
    await expect(
      costDialog.getByText(el.processingGateTotalBalanceLabel),
    ).toBeVisible();
    await expect(
      costDialog.getByText(el.processingGateLocalChargeNote),
    ).toBeVisible();
    await expect(
      costDialog.getByText(el.processingGateBalanceLabel),
    ).toHaveCount(0);
    await expect(
      costDialog.getByRole("button", {
        name: new RegExp(el.processingGateConfirm.replace("{cost}", "\\d+")),
      }),
    ).toBeEnabled();
  });

  test("authoritative 30-to-60 quote change requires a second explicit confirmation", async ({
    page,
  }) => {
    ...
    await page.route("**/videos/process*", async (route) => {
      processRequests += 1;
      const encodedMetadata = route.request().headers()[
        "x-gsubs-upload-metadata"
      ];
      processMetadata.push(
        JSON.parse(
    ...
    expect(processMetadata[0]).toEqual(
    ...
    expect(processMetadata[1]).toEqual(
      expect.objectContaining({
        authorized_credits: 60,
        filename: "demo_output.mp4",
      }),
    );
    const { authorized_credits: firstCredits, ...firstSettings } =
      processMetadata[0];
  ...
  test("complete flow: upload -> processing -> completed -> download", async ({
    page,
  }) => {
    ...
    page.on("request", (request) => {
      ...
      if (
        url.pathname.endsWith(
          "/static/artifacts/job-123/processed_720x1280.mp4",
        ) &&
    ...
    expect(
      (await startButton.innerText()).match(/([\d,.]+)\s*$/)?.[1],
    ).toBeTruthy();
  ...
  test("slow mobile upload uses progress-capable XHR, stays responsive, and submits once", async ({
    page,
  }) => {
  ...
  test("guest keeps the uploaded file through login and sees cost before start", async ({
    page,
  }) => {
    ...
    await authDialog
      ...
      .click();
    const legalNotice = authDialog.locator(
    ...
    await authDialog
      .getByRole("button", { name: el.processingGateUseLogin })
      .click();
    ...
    const passwordInput = page.getByLabel(el.loginPasswordLabel, {
      exact: true,
    });
    ...
    await page
      .getByRole("button", { name: el.processingGateLoginSubmit })
      .click();

    await expect(
      page.getByRole("dialog", { name: el.processingGateCostTitle }),
    ).toBeVisible();
    await expect(
    ...
    await page
      .getByRole("dialog", { name: el.processingGateCostTitle })
      .getByRole("button", {
        name: new RegExp(el.processingGateConfirm.replace("{cost}", "\\d+")),
      })
      .click();
