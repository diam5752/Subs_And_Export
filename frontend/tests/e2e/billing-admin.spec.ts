...
test('admin links a completed refund and AADE correction before accepting a withdrawal', async ({ page }) => {
  const accountingPayloads: unknown[] = [];
  const resolutionPayloads: unknown[] = [];
  await mockApi(page);
  await mockManualRefundReview(
    page,
    accountingPayloads,
    resolutionPayloads,
  );
  ...
  await expect(withdrawalCard.getByRole('radio', {
  ...
  const adjustmentSeries = refundCard.locator(
    'input[name="adjustmentSeries"]',
  );
  const adjustmentAa = refundCard.locator('input[name="adjustmentAa"]');
  const adjustmentMark = refundCard.locator(
    'input[name="adjustmentMark"]',
  );
  ...
  await refundCard.getByLabel(
    el.adminBillingFinalRefundActionsConfirm,
    { exact: true },
  ).check();
  await refundCard.getByRole('button', {
    name: el.adminBillingRecordRefundEvidence,
  }).click();

  await expect.poll(() => accountingPayloads).toEqual([{
  ...
  await accept.check();
  await withdrawalCard.locator(
    'select[name="adjustmentId"]',
  ).selectOption(adjustmentId);
  const explanation = (
    'Το refund και το διορθωτικό ολοκληρώθηκαν και ελέγχθηκαν χειροκίνητα.'
  );
  await withdrawalCard.getByLabel(
    el.adminBillingWithdrawalCustomerExplanation,
    { exact: true },
  ).fill(explanation);
  await withdrawalCard.getByLabel(
    el.adminBillingFinalWithdrawalReviewConfirm,
    { exact: true },
  ).check();
  await withdrawalCard.getByRole('button', {
