...
test.describe("Job Polling E2E", () => {
  ...
  test("dashboard can display processing jobs from history", async ({
    page,
  }) => {
  ...
  test("unauthenticated state keeps the public workspace available", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("link", { name: el.guestSignIn }),
    ).toBeVisible();
  });
