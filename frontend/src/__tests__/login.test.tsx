...
describe("LoginPage", () => {
  ...
  it("renders login form by default", () => {
    ...
    expect(
      within(homeLink).getByRole("img", { name: "gsubs" }),
    ...
    expect(
      screen.getByPlaceholderText("loginEmailPlaceholder"),
    ).toBeInTheDocument();
  ...
  it("keeps privacy and terms reachable before authentication", () => {
    ...
    expect(
      screen.getByRole("link", { name: "legalTermsLink" }),
    ).toHaveAttribute("href", "/terms");
    expect(
      screen.getByRole("link", { name: "legalPrivacyLink" }),
  ...
  it("keeps Messenger users out of the unsupported Google WebView flow", async () => {
    ...
    try {
      ...
      const fallback = await screen.findByTestId(
        "google-embedded-browser-fallback",
      );
  ...
  it("aborts a pending nonce request when the login page unmounts", async () => {
    ...
    (api.getGoogleAuthNonce as jest.Mock).mockImplementation(
      (signal?: AbortSignal) => {
  ...
  it("expires the Google button from expires_in and never posts its stale credential", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "loginGoogleReload" }),
    ).toBeVisible();
  ...
  it("does not initialize GIS when the nonce expires while its script is loading", async () => {
    ...
    (loadGoogleIdentityScript as jest.Mock).mockReturnValue(
  ...
  it("localizes a server nonce rejection and never retries the submitted credential", async () => {
    ...
    mockGoogleLogin.mockRejectedValue(
      new Error("Google login nonce is required."),
    );
    render(<LoginPage />);
    ...
    expect(
      screen.queryByText("Google login nonce is required."),
  ...
  it("shows one localized availability notice when Google initialization fails", async () => {
    ...
    expect(
      screen.queryByText("Google auth unavailable"),
    ).not.toBeInTheDocument();
  ...
  it("does not continue authentication after the Google control unmounts", async () => {
    ...
    mockGoogleLogin.mockReturnValue(
      new Promise<void>((resolve) => {
