...
describe("i18n provider defaults and persistence", () => {
  it("keeps locale dictionaries in sync", () => {
    expect(Object.keys(messages.el).sort()).toEqual(
    ...
    expect(messages.el.transcriptionMissingWordTimestamps).not.toContain(
      "ElevenLabs",
    );
  ...
  it("switches to English through the toggle and persists the locale", async () => {
    ...
    expect(screen.getByTestId("hero-copy")).toHaveTextContent(
      messages.el.heroTitle,
    );
...
describe("localized pages", () => {
  it("shows Greek strings across dashboard by default", async () => {
    ...
    expect(
      await screen.findByText("Υπότιτλοι. Χωρίς κόπο."),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Ρυθμίσεις μηχανής" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Credits: 1,000" }),
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Προφίλ" })).toHaveAttribute(
      "title",
  ...
  it("renders English copy when locale is set to en", async () => {
    ...
    try {
      ...
      expect(
        (await screen.findAllByText("Captions. Effortless.")).length,
      ).toBeGreaterThan(0);
