  ...
  return (
    ...
    >
      <div className="min-w-0">
        ...
        <p className="mt-1 text-sm font-bold">{t('betaLaunchAwardTitle', { count })}</p>
        <p className="mt-0.5 text-xs text-sky-800">{t('betaLaunchAwardBody')}</p>
      </div>
      <button
        type="button"
        onClick={onDismiss}
...
export default function DashboardPage() {
  const {
    user, isLoading, sessionUnavailable, betaCreditsAwarded,
    logout, refreshUser, retrySession,
    ...
    <div className="app-shell min-h-dvh relative overflow-x-hidden">
      ...
      >
        ...
        >
          ...
          {user && betaCreditsAwarded > 0 && (
            <BetaLaunchCreditAward
              count={betaCreditsAwarded}
              onDismiss={dismissBetaCreditsAwarded}
            />
          )}
