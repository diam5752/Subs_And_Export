...
jest.mock('@/lib/observability', () => ({ fetchObservabilitySnapshot: jest.fn() }));
jest.mock('@/components/BrandLogo', () => ({ BrandLogo: () => <span>GSUBS</span> }));
...
const snapshot = {
    ...
    actions: [{ name: 'export_completed', outcome: 'succeeded', export_format: '1080p', count: 4 }],
    errors: [{ kind: 'api_error', name: 'http_5xx', route: 'studio', status_code: 503, count: 1 }],
    recent: [{ ts: 1_788_048_000, kind: 'action', name: 'export_completed', route: 'studio', auth_state: 'authenticated' }],
};

describe('ObservabilityAdminPage', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        (useAuth as jest.Mock).mockReturnValue({ user: { id: 'owner' }, isLoading: false });
    ...
    it('renders live counts, actions, jobs, and sanitized errors', async () => {
        ...
        expect(screen.getByText(/export_completed · succeeded · 1080p/)).toBeInTheDocument();
        expect(screen.getByText(/api_error · http_5xx · studio · 503/)).toBeInTheDocument();
    ...
    it('fails closed for a non-allowlisted account', async () => {
        ...
        expect(await screen.findByRole('alert')).toHaveTextContent('observabilityForbidden');
    });
});
