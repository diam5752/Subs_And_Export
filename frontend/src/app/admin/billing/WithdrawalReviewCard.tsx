...
type WithdrawalReviewCardProps = {
  ...
  onResolved: (resolution: BillingWithdrawalResolutionResponse) => void;
};
...
export function WithdrawalReviewCard({
  ...
  const hasCompletedManualActions = review.available_adjustments.length > 0;
    ...
    try {
      const resolution = await api.resolveBillingWithdrawal(
        review.withdrawal_id,
        {
          ...
          adjustment_id:
            values.decision === "accepted_refunded"
              ? values.adjustmentId
              : null,
          customer_explanation: values.customerExplanation,
    ...
    >
      ...
      <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-7">
        ...
        <div>
          ...
          <p className="mt-1 text-sm">
            {formatDateTime(review.contract_concluded_at, locale)}
          </p>
        </div>
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted)]">
        ...
        <div>
          ...
          <p className="mt-1 break-all text-sm">{review.confirmation_email}</p>
        </div>
      </div>
      ...
      >
        <fieldset className="space-y-3">
          ...
          <label className="flex items-start gap-3 rounded-2xl border border-emerald-300 bg-emerald-50 p-4 text-sm leading-6">
            <input
              ...
              {...register("decision", {
                required: t("adminBillingWithdrawalDecisionRequired"),
              })}
            />
            <span>
          ...
          {!hasCompletedManualActions && (
            ...
            >
              {t("adminBillingWithdrawalNeedsRefundEvidence")}
            </p>
          )}
          ...
          <label className="grid gap-2 text-sm font-semibold">
            ...
            <select
              ...
              {...register("adjustmentId", {
                required: t("adminBillingWithdrawalAdjustmentRequired"),
              })}
            >
              <option value="">
                {t("adminBillingWithdrawalSelectAdjustment")}
              </option>
              {review.available_adjustments.map((adjustment) => (
                <option
                ...
                >
                  ...
                  {" · "}
                  {(adjustment.amount_cents / 100).toFixed(2)}{" "}
        ...
        <label className="grid gap-2 text-sm font-semibold">
          ...
          <textarea
            aria-label={t("adminBillingWithdrawalCustomerExplanation")}
            rows={5}
            maxLength={1000}
            className="rounded-xl border border-[var(--border-strong)] p-3 leading-6"
            {...register("customerExplanation", {
              required: t("adminBillingWithdrawalExplanationInvalid"),
              minLength: {
                value: 20,
                message: t("adminBillingWithdrawalExplanationInvalid"),
              },
              maxLength: {
                value: 1000,
                message: t("adminBillingWithdrawalExplanationInvalid"),
              ...
              validate: (value) =>
                value.trim() === value ||
                t("adminBillingWithdrawalExplanationInvalid"),
            })}
          />
        ...
        <label className="flex items-start gap-3 rounded-2xl border border-[var(--border)] bg-[#fafaf8] p-4 text-sm font-semibold leading-6">
          <input
            ...
            {...register("finalManualReviewConfirmed", {
              required: t("adminBillingFinalWithdrawalReviewConfirm"),
            })}
          />
          <span>{t("adminBillingFinalWithdrawalReviewConfirm")}</span>
        </label>
