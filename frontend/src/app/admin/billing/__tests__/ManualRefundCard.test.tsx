...
const ORIGINAL_ISSUED_AT = (
    Date.parse('2026-02-15T08:30:00Z') / 1000
);
const ADJUSTMENT_ISSUED_AT = (
    Date.parse('2026-02-15T09:30:00Z') / 1000
);
...
function fillAdjustmentForm(): void {
    const adjustment = screen.getByText(
        'adminBillingAdjustmentDocumentTitle',
    ).closest('section');
    if (!adjustment) {
        throw new Error('Adjustment section was not rendered');
    }
    fireEvent.change(within(adjustment).getByLabelText(
    ...
    fireEvent.change(within(adjustment).getByLabelText(
        'adminBillingMarkRepeat',
    ), {
        target: { value: ADJUSTMENT_MARK },
    });
    fireEvent.change(within(adjustment).getByLabelText(
        'adminBillingIssuedAt',
    ), {
...
describe('ManualRefundCard', () => {
    ...
    it('starts with every adjustment identity field blank', () => {
        ...
        expect(within(adjustment as HTMLElement).getByLabelText(
            'adminBillingDocumentType',
        )).toHaveValue('');
        expect(within(adjustment as HTMLElement).getByLabelText(
            'adminBillingSeries',
    ...
    it('records exact existing Stripe and AADE evidence without defaults', async () => {
        ...
        const original = screen.getByText(
            'adminBillingOriginalDocumentTitle',
        ).closest('section');
        if (!original) {
            throw new Error('Original section was not rendered');
        }
        fireEvent.change(within(original).getByLabelText(
            'adminBillingAa',
        ), { target: { value: '7' } });
        fireEvent.change(within(original).getByLabelText(
            'adminBillingMark',
    ...
    it('requires the exact final confirmation and never retries a failed write', async () => {
        recordManualRefund.mockRejectedValueOnce(
            new Error('Response lost'),
        );
        render(
            <ManualRefundCard
                review={refundReview(originalInvoice({
