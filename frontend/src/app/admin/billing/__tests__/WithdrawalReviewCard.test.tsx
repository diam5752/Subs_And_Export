...
const resolveWithdrawal = (
    api.resolveBillingWithdrawal as jest.MockedFunction<
...
function resolution(
    ...
    return {
        ...
        purchase_id: PURCHASE_ID,
        decision,
        reason_code: decision === 'accepted_refunded'
            ? 'statutory_right_accepted'
            : 'request_not_eligible',
        adjustment_id: decision === 'accepted_refunded'
            ? ADJUSTMENT_ID
            : null,
        resolved_at: 1_800_000_300,
        resolution_sha256: '5'.repeat(64),
        resolution_url: (
            `/billing/purchases/${PURCHASE_ID}/withdrawal-resolution`
        ),
    };
}
...
describe('WithdrawalReviewCard', () => {
    ...
    it('accepts only with linked completed manual evidence', async () => {
        ...
        fireEvent.change(screen.getByLabelText(
            'adminBillingWithdrawalAdjustmentEvidence',
        ), {
    ...
    it('requires a final choice, explanation, confirmation and never retries', async () => {
        resolveWithdrawal.mockRejectedValueOnce(
        ...
        expect(await screen.findByText(
            'adminBillingWithdrawalResolveError',
        )).toBeInTheDocument();
        expect(resolveWithdrawal).toHaveBeenCalledTimes(1);
