...
const recordIssuedDocument =
  ...
  >;
const listPendingRefunds = api.listPendingBillingRefunds as jest.MockedFunction<
...
function pendingResponse<T>(items: T[], nextCursor: string | null = null) {
  return {
    items,
    count: items.length,
...
async function renderPendingInvoice(
  ...
  await screen.findByTestId(`billing-admin-invoice-${invoice.invoice_id}`);
}
...
function fillDocumentForm(overrides: FormOverrides = {}) {
  ...
  if (values.confirmed) {
    fireEvent.click(screen.getByLabelText("adminBillingFinalDocumentConfirm"));
  }
}
...
describe("BillingAdminPage", () => {
  ...
  it("shows sign-in without requesting the queue when unauthenticated", async () => {
    ...
    expect(await screen.findByText("adminBillingSignIn")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "loginSubmit" })).toHaveAttribute(
      "href",
      "/login",
    );
    expect(listPendingInvoices).not.toHaveBeenCalled();
  ...
  it("renders the forbidden state only for a typed 403 API error", async () => {
    listPendingInvoices.mockRejectedValueOnce(new ApiError("Forbidden", 403));
  ...
  it("renders only the minimal typed reconciliation DTO", async () => {
    ...
    expect(
      screen.getByText("1 Example Street, 10558 Athens, GR"),
    ).toBeInTheDocument();
  });
  ...
  it("loads the independent refund and withdrawal review queues", async () => {
    ...
    expect(
      await screen.findByTestId(`billing-admin-refund-${"r".repeat(32)}`),
    ).toBeInTheDocument();
    expect(screen.getByText("re_completed_refund")).toBeInTheDocument();
    expect(
      screen.getByTestId(`billing-admin-withdrawal-${"w".repeat(32)}`),
    ).toBeInTheDocument();
  ...
  it("locks the verified MizAI document type and series defaults", async () => {
    ...
    expect(screen.getByLabelText("adminBillingDocumentType")).toHaveValue(
      "11.2",
    );
    expect(screen.getByLabelText("adminBillingDocumentType")).toHaveAttribute(
      "readonly",
    );
    expect(screen.getByLabelText("adminBillingSeries")).toHaveValue("0");
    expect(screen.getByLabelText("adminBillingSeries")).toHaveAttribute(
      "readonly",
    );
    expect(screen.getByText("adminBillingMizaiBaseline")).toBeInTheDocument();
  });
  ...
  it("shows an already-issued document as read-only", async () => {
    ...
    expect(screen.getByText("adminBillingIssuedReview")).toBeInTheDocument();
  ...
  it("submits one exact canonical payload and removes the recorded row", async () => {
    ...
    expect(
      screen.queryByTestId("billing-admin-invoice-invoice-1"),
    ).not.toBeInTheDocument();
  });
  ...
  it("reports a failed write once without retrying it automatically", async () => {
    ...
    expect(recordIssuedDocument).toHaveBeenCalledTimes(1);
    expect(
      screen.getByTestId("billing-admin-invoice-invoice-1"),
    ).toBeInTheDocument();
  });
  ...
  it("deduplicates appended pages and refreshes from the beginning", async () => {
    ...
    listPendingInvoices
      .mockResolvedValueOnce(pendingResponse([firstInvoice], "cursor-1"))
      .mockResolvedValueOnce(pendingResponse([firstInvoice, secondInvoice]))
      .mockResolvedValueOnce(pendingResponse([refreshedInvoice]));
    ...
    expect(
      screen.getAllByTestId("billing-admin-invoice-invoice-1"),
    ).toHaveLength(1);
    ...
    expect(
      screen.queryByTestId("billing-admin-invoice-invoice-2"),
    ).not.toBeInTheDocument();
