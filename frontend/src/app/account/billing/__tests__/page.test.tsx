...
async function openWithdrawalForm() {
    ...
    return screen.getByRole('button', { name: 'Confirm withdrawal' })
...
describe('BillingAccountPage', () => {
    ...
    it('submits a withdrawal once, disables the form, and reloads the vault', async () => {
        let resolveWithdrawal: (
            response: typeof withdrawalResponse,
        ) => void = () => undefined;
        ...
        ).toHaveTextContent('Withdrawal pending manual review');
        await waitFor(() => {
    ...
    it.each([
        [new Error('Withdrawal service unavailable'), 'Withdrawal service unavailable'],
        [{ reason: 'unexpected' }, 'The withdrawal could not be submitted.'],
    ])('keeps the form open and reports withdrawal failures', async (
        failure,
        message,
    ) => {
    ...
    it('reports electronic-route unavailability without claiming expiry', async () => {
        ...
        expect(screen.queryByText(/expired|window|14.day/i)).not.toBeInTheDocument();
        expect(
