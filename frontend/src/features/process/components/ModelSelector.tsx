    ...
    return useMemo(() => (
        <div id="model-selection-step" className="card space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 relative z-50">
                <div
                className={`relative flex items-center gap-3 transition-all duration-300 group/step ${currentStep !== 1 ? 'opacity-100 hover:scale-[1.005]' : 'opacity-100 scale-[1.01]'}`}
                >
                {/* Main Step Toggle Button (Background) */}
                <button
                    type="button"
                    onClick={handleStepClick}
                    className="absolute inset-0 w-full h-full rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] z-0"
                    aria-label={t('modelSelectTitle') || 'Pick a Model'}
                />

                <span className={`relative z-10 pointer-events-none flex items-center justify-center px-4 py-1 rounded-full border font-mono text-sm font-bold tracking-widest shadow-sm transition-all duration-500 shrink-0 ${currentStep === 1
                ...
                <button
                    type="button"
                    className="group/info relative z-20 shrink-0 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] cursor-help"
                        // @ts-expect-error - 'modelInfo' is missing from translation types but provided by fallback or en.json
                    ...
                    <svg
                    className={`w-5 h-5 text-[var(--muted)] transition-transform duration-300 shrink-0 relative z-10 pointer-events-none ${isExpanded ? 'rotate-180' : ''}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
