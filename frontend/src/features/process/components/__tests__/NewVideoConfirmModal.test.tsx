...
jest.mock("@/context/I18nContext", () => ({
  useI18n: () => ({
    t: (key: keyof typeof el) =>
      mockMissingNewVideoTranslations ? "" : (el[key] ?? key),
  }),
}));
...
describe("NewVideoConfirmModal", () => {
  ...
  it("accurately explains the temporary History window", () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Νέο project;" }),
    ).toHaveTextContent(
      "Το project θα μείνει στο Ιστορικό μέχρι την αυτόματη διαγραφή του",
    );
    expect(
      screen.queryByText(/δεν μπορεί να αναιρεθεί/i),
  ...
  it("focuses the safe action instead of the destructive confirmation", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Συνέχιση επεξεργασίας" }),
    ).toHaveFocus();
    expect(
      screen.getByRole("button", { name: "Νέο project" }),
    ).not.toHaveFocus();
  });
  ...
  it("supports Escape, cancel, and explicit confirmation", () => {
    ...
    render(
      <NewVideoConfirmModal isOpen onClose={onClose} onConfirm={onConfirm} />,
    );
  ...
  it("keeps safe English labels when translations are unavailable", () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Start a new project?" }),
    ).toHaveTextContent("This closes the current editing view.");
    expect(
