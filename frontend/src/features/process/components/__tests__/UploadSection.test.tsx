...
jest.mock("next/image", () => ({
  __esModule: true,
  default: (
    allProps: React.ImgHTMLAttributes<HTMLImageElement> & {
      fill?: boolean;
      unoptimized?: boolean;
      sizes?: string;
    },
...
describe("UploadSection", () => {
  ...
  it("shows an error when the video duration cannot be read", async () => {
    contextValue.selectedFile = new File(["video"], "broken.mp4", {
      type: "video/mp4",
    });
    ...
    expect(contextValue.setPreviewVideoUrl).toHaveBeenCalledWith(
      "blob:preview",
    );
  ...
  it("shows an error when the selected video is too long", async () => {
    contextValue.selectedFile = new File(["video"], "long.mp4", {
      type: "video/mp4",
    });
    (validateVideoAspectRatio as jest.Mock).mockResolvedValueOnce({
      width: 1080,
  ...
  it("accepts a video at the three-minute launch limit", async () => {
    contextValue.selectedFile = new File(["video"], "three-minute-clip.mp4", {
      type: "video/mp4",
    });
    (validateVideoAspectRatio as jest.Mock).mockResolvedValueOnce({
      width: 1080,
    ...
    await waitFor(() => {
      expect(contextValue.setVideoInfo).toHaveBeenCalledWith(
  ...
  it("keeps processing disabled until the selected file validation settles", async () => {
    contextValue.selectedFile = new File(["video"], "delayed.mp4", {
      type: "video/mp4",
    });
    ...
    (validateVideoAspectRatio as jest.Mock).mockReturnValueOnce(
      new Promise((resolve) => {
  ...
  it("does not reopen a settled same-file validation when context callbacks change", async () => {
    ...
    contextValue.selectedFile = new File(["video"], "too-long.mp4", {
      type: "video/mp4",
    });
    (validateVideoAspectRatio as jest.Mock).mockResolvedValueOnce({
      width: 1080,
    ...
    await waitFor(() => {
      expect(
        screen.getAllByText("uploadDurationTooLong").length,
      ).toBeGreaterThan(0);
      expect(startButton).toBeDisabled();
    });
    ...
    expect(validateVideoAspectRatio).toHaveBeenCalledTimes(1);
    expect(startButton).toBeDisabled();
    expect(screen.getAllByText("uploadDurationTooLong").length).toBeGreaterThan(
      0,
    );
  });

  it("aborts stale metadata validation when the selected file changes", () => {
    contextValue.selectedFile = new File(["first"], "first.mp4", {
      type: "video/mp4",
    });
    (validateVideoAspectRatio as jest.Mock).mockReturnValue(
      new Promise(() => undefined),
    );
    const view = renderUpload();
    const firstSignal = (validateVideoAspectRatio as jest.Mock).mock
      .calls[0][1] as AbortSignal;
    expect(firstSignal.aborted).toBe(false);

    contextValue.selectedFile = new File(["second"], "second.mp4", {
      type: "video/mp4",
    });
    view.rerender(<UploadSection />);
    ...
    expect(
      (validateVideoAspectRatio as jest.Mock).mock.calls[1][1],
  ...
  it("handles drag-and-drop uploads and unlock-step reset", () => {
    ...
    expect(contextValue.onFileSelect).toHaveBeenCalledWith(
      expect.objectContaining({ name: "drop.mp4" }),
    );
  ...
  it("keeps the input summary aligned with Step 1 without a stale nested step label", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "inputVideoTitle" }),
    ).toBeInTheDocument();
    ...
    const summaryToggle = screen.getByRole("button", {
      name: "inputVideoSummaryToggle",
    });
  ...
  it("uses duration pricing regardless of provider and recognizes a matching pro job", () => {
    ...
    expect(
      screen.getByRole("button", { name: /startProcessing 30/i }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: "viewResults" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /startProcessing/i }),
    ).not.toBeInTheDocument();
  });

  it("keeps 3 minutes active and marks the larger launch tiers as coming soon", async () => {
    contextValue.selectedFile = new File(["video"], "priced.mp4", {
      type: "video/mp4",
    });
    contextValue.videoInfo = {
      width: 1080,
    ...
    expect(
      within(pricing).getAllByText("videoCreditPricingComingSoon"),
    ).toHaveLength(2);
    expect(pricing.querySelectorAll('[data-available="false"]')).toHaveLength(
      2,
    );
    expect(pricing.querySelector('[data-active="true"]')).toHaveTextContent(
      "30 creditsLabel",
    );
    const startButton = screen.getByRole("button", {
      name: /startProcessing 30/i,
    });
    await waitFor(() => {
      expect(startButton).toBeEnabled();
    });
