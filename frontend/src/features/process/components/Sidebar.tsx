...
function changeTabWithKeyboard(
  ...
  let next: "transcript" | "styles" =
    props.activeTab === "transcript" ? "styles" : "transcript";
  if (event.key === "Home") next = "transcript";
  if (event.key === "End") next = "styles";
  props.onChange(next);
  event.currentTarget.querySelector<HTMLButtonElement>(`#tab-${next}`)?.focus();
}

function SidebarTabIcon({ transcript }: { transcript: boolean }) {
  return (
    <svg
...
const SidebarTabs = memo((props: SidebarTabsProps) => (
  <div className="editor-tabs-sticky">
    <div
      ...
      onKeyDown={(event) => changeTabWithKeyboard(event, props)}
    >
      {(["transcript", "styles"] as const).map((tab) => (
        <button
          key={tab}
          type="button"
          role="tab"
          ...
          aria-label={
            tab === "transcript" ? props.transcriptLabel : props.stylesLabel
          }
          aria-selected={props.activeTab === tab}
          aria-controls={`panel-${tab}`}
          onClick={() => props.onChange(tab)}
          className={`editor-tab ${props.activeTab === tab ? "editor-tab-active" : ""}`}
        >
          <SidebarTabIcon transcript={tab === "transcript"} />
          <span
            className={
              tab === "transcript"
                ? "editor-tab-label-full truncate"
                : "truncate"
            }
          >
            {tab === "transcript" ? props.transcriptLabel : props.stylesLabel}
