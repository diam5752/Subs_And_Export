...
export const StepIndicator = React.memo(function StepIndicator({ currentStep, steps, onStepClick, maxStep }: StepIndicatorProps) {
    ...
    return (
        <nav
            aria-label={t('progressSteps') || 'Progress'}
            className="w-full max-w-4xl mx-auto mb-6 px-8 sm:px-12"
        >
            <div className="relative">
                ...
                <ol className="relative flex items-center justify-between z-10">
                    {steps.map((step) => {
                        ...
                        const labelText = t('stepLabel', { n: step.id }) || `Step ${step.id}`;

                        // Accessibility strategy:
                        // If clickable (interactive): acts as a button. We use aria-label on the container and hide inner text to avoid redundancy.
                        // If NOT clickable (static): acts as a static list item. We do NOT use aria-label (it's ignored on divs) and we show inner text.

                        const containerProps = isClickable ? {
                            role: "button",
                            tabIndex: 0,
                            "aria-label": `${labelText}: ${step.label}`,
                            "aria-current": isActive ? "step" as const : undefined,
                            onKeyDown: handleKeyDown,
                            onClick: () => onStepClick?.(step.id)
                        } : {
                            "aria-current": isActive ? "step" as const : undefined,
                            // No role, no tabIndex, no aria-label for static content
                        };
