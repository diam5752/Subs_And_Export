...
jest.mock("@/context/I18nContext", () => ({
  useI18n: () => ({
    t: (key: string, params?: { n?: number }) =>
      key === "stepLabel" ? `Step ${params?.n}` : key,
  }),
}));
...
describe("StepIndicator", () => {
  it("shows a compact active, unlocked and locked workflow state", () => {
    ...
    expect(screen.getByTestId("workflow-stepper")).toHaveAttribute(
      "aria-label",
      "workflowProgressLabel",
    );
    expect(
      screen.getByRole("button", { name: /Step 2 Captions/ }),
    ).toHaveAttribute("aria-current", "step");
    expect(screen.getByRole("button", { name: /Step 1 Upload/ })).toBeEnabled();
    expect(
      screen.getByRole("button", { name: /Step 3 Export/ }),
    ).toBeDisabled();
  });
