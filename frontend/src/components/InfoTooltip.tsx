...
function TooltipBubble({
  ...
  return (
    <div
      ...
      style={{ left: layout?.left ?? -9999, top: layout?.top ?? -9999 }}
    >
      <div
        aria-hidden="true"
        className={`absolute h-3 w-3 -translate-x-1/2 rotate-45 bg-[var(--surface-elevated)] ${
...
export function InfoTooltip({ ariaLabel, children }: InfoTooltipProps) {
  ...
  const updateLayout = useCallback(() => {
    ...
    const clampedCenterX = Math.min(
      Math.max(anchorCenterX, minCenterX),
      maxCenterX,
    );
