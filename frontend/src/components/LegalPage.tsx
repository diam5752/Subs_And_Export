...
const sections: Record<LegalPageKind, LegalSection[]> = {
  privacy: [
    ...
    {
      title: "privacyFinancialRetentionTitle",
      body: "privacyFinancialRetentionBody",
    },
...
export function LegalPage({ kind }: LegalPageProps) {
  ...
  const titleKey: MessageKey = isPrivacy
    ? "privacyPageTitle"
    : "termsPageTitle";
  const introKey: MessageKey = isPrivacy
    ? "privacyPageIntro"
    : "termsPageIntro";
  const kickerKey: MessageKey = isPrivacy
    ? "legalPrivacyKicker"
    : "legalTermsKicker";
    ...
    <div className="min-h-dvh bg-[#f7f7f5] text-[var(--foreground)]">
      ...
      <main className="mx-auto w-full max-w-3xl px-5 py-10 sm:px-8 sm:py-16">
        ...
        <article>
          ...
          <h1 className="max-w-2xl text-4xl font-extrabold tracking-[-0.045em] sm:text-6xl">
            {t(titleKey)}
          </h1>
          <p className="mt-6 max-w-2xl text-base leading-7 text-[var(--muted)] sm:text-lg sm:leading-8">
            {t(introKey)}
          </p>
          <p className="mt-5 text-xs font-semibold uppercase tracking-[0.12em] text-[#95989f]">
            {t("legalLastUpdated")}
          </p>
          ...
          <div className="mt-12 border-t border-[var(--border)]">
            {visibleSections.map((section) => (
              ...
              >
                ...
                <p className="mt-4 text-[15px] leading-7 text-[#5f636b] sm:text-base sm:leading-8">
                  {t(section.body)}
                </p>
                {isPrivacy && section.body === "privacyProvidersBody" && (
                  <div className="mt-5 text-sm leading-6 text-[#5f636b]">
                    <p className="font-semibold">
                      {t("privacyElevenLabsLinksIntro")}
                    </p>
