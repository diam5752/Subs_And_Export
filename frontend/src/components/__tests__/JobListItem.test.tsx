...
describe("JobListItem", () => {
  ...
  it("shows how long the temporary project remains available", () => {
    ...
    render(
      <JobListItem
        ...
        t={(key, params) =>
          key === "availableForHours" ? `available ${params?.hours}h` : key
        }
      />,
    );
  ...
  it("requests a secure download and shows accessible actions when completed", () => {
    ...
    expect(screen.getByLabelText("view test-video.mp4")).toHaveClass(
      "min-h-11",
    );
  ...
  it("handles selection toggle in selection mode", () => {
    ...
    fireEvent.click(
      screen.getByText("test-video.mp4").closest("div")!.parentElement!,
    );
  ...
  it("shows loading state with accessible label", () => {
    ...
    expect(
      screen.getByLabelText("deleting test-video.mp4"),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "deleting test-video.mp4" }),
  ...
  it("shows the expired badge instead of actions for expired jobs", () => {
    ...
    expect(
      screen.queryByLabelText("view test-video.mp4"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText("download test-video.mp4"),
    ).not.toBeInTheDocument();
  });

  it("has correct accessibility attributes", () => {
    const { rerender } = render(
      <JobListItem {...mockProps} selectionMode={true} />,
    );
    ...
    // Delete button label
    expect(
      screen.getByLabelText("deleteJob test-video.mp4"),
    ).toBeInTheDocument();
  });
});
