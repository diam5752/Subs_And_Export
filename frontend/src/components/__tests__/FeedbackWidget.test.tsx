...
describe("FeedbackWidget", () => {
  ...
  it("opens as an accessible, scroll-locked dialog and restores focus on Escape", () => {
    ...
    expect(
      screen.queryByRole("dialog", { name: "feedbackTitle" }),
  ...
  it("submits the selected category and path without query data", async () => {
    ...
    try {
      ...
      fireEvent.click(
        screen.getByRole("radio", { name: "feedbackCategoryBug" }),
      );
      ...
      await waitFor(() =>
        expect(api.createProductFeedback).toHaveBeenCalledWith({
          category: "bug",
  ...
  it("keeps a failed message available for retry", async () => {
    jest.useFakeTimers();
    try {
      (api.createProductFeedback as jest.Mock).mockRejectedValue(
        new Error("offline"),
      );
      render(<FeedbackWidget />);
      fireEvent.click(screen.getByRole("button", { name: "feedbackOpen" }));
  ...
  it("validates short messages before any feedback request is sent", async () => {
    ...
    try {
      ...
      await act(async () => {
        fireEvent.submit(
          screen.getByLabelText("feedbackMessageLabel").closest("form")!,
        );
      ...
      expect(
        await screen.findByText("feedbackMessageTooShort"),
      ).toBeInTheDocument();
  ...
  it("supports anonymous success, sending another message, and finishing", async () => {
    ...
    try {
      ...
      fireEvent.click(
        screen.getByRole("button", { name: "feedbackSendAnother" }),
      );
      expect(screen.getByLabelText("feedbackMessageLabel")).toHaveValue("");
      expect(
        screen.getByRole("button", { name: "feedbackWaitMoment" }),
      ).toBeDisabled();
      fireEvent.change(screen.getByLabelText("feedbackMessageLabel"), {
