...
describe("SubtitleOverlay", () => {
  ...
  it("renders three explicit, stable rows without scaling the active word", () => {
    ...
    try {
      ...
      expect(
        lines.every((line) => line.classList.contains("whitespace-nowrap")),
      ).toBe(true);
    } finally {
  ...
  it("opens and controls the synchronized editor directly on the active subtitle", () => {
    ...
    const inlineTrigger = screen.getByRole("button", {
      name: "Edit active subtitle",
    });
    ...
    expect(
      document.querySelector(".subtitle-edit-affordance"),
