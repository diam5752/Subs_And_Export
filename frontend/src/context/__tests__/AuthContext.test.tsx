...
function AuthHarness() {
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => void register("new@example.com", "secret", "New User")}
      >
        register
      </button>
      <button
        type="button"
        onClick={() => void googleLogin("signed-google-id-token")}
      >
        google
      </button>
      <button
...
describe("AuthContext", () => {
  ...
  it("offers recovery without clearing a token after a transient initial session failure", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("loading")).toHaveTextContent("false");
      expect(screen.getByTestId("session-unavailable")).toHaveTextContent(
        "true",
      );
    });
  ...
  it("surfaces and dismisses the one-time Beta credit award", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("beta-credits-awarded")).toHaveTextContent(
        "30",
      );
    });
  ...
  it("clears local private state only after server revocation succeeds", async () => {
    ...
    expect(
      (api.revokeSession as jest.Mock).mock.invocationCallOrder[0],
  ...
  it("preserves an established session on refresh network and server errors", async () => {
    ...
    (api.getCurrentUser as jest.Mock).mockRejectedValueOnce(
      new Error("network unavailable"),
    );
    fireEvent.click(screen.getByRole("button", { name: "refresh" }));
