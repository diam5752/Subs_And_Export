...
const mockUseAuth = jest.fn((): { user: typeof mockUser | null } => ({
  user: mockUser,
}));
...
describe("useJobs Hook", () => {
  ...
  it("handles API errors", async () => {
    (api.getJobsPaginated as jest.Mock).mockRejectedValue(
  ...
  it("uses the localized fallback for non-Error failures", async () => {
    ...
    mockT.mockImplementation((key) =>
      key === "jobsErrorFallback" ? "Fallback Error" : key,
    );
    ...
    (api.getJobsPaginated as jest.Mock).mockResolvedValue({
      items: [
        { id: "pending", status: "pending" },
        {
          id: "missing",
          status: "completed",
          result_data: { files_missing: true },
        },
        {
          id: "available",
          status: "completed",
