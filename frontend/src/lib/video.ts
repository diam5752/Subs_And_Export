...
type IsoTopLevelScan = {
  sawFileType: boolean;
  movieRange: { start: number; size: number } | null;
};
...
function readIsoBox(
  ...
  if (start < 0 || containerEnd > view.byteLength || start + 8 > containerEnd)
    return null;
...
function parseMovieDuration(
  ...
  const { timescaleOffset, durationOffset } = movieDurationOffsets(
    contentStart,
    version,
  );
  if (timescaleOffset + 4 > box.end) return null;
  const timescale = view.getUint32(timescaleOffset);
  const duration = readMovieDurationValue(
    view,
    version,
    durationOffset,
    box.end,
  );
  if (!isUsableMovieDuration(timescale, duration, version)) return null;
...
function movieDurationOffsets(contentStart: number, version: number) {
  if (version === 1) {
    return {
      timescaleOffset: contentStart + 20,
      durationOffset: contentStart + 24,
    };
  }
  return {
    timescaleOffset: contentStart + 12,
    durationOffset: contentStart + 16,
  };
}

function readMovieDurationValue(
  view: DataView,
  version: number,
  ...
  if (version === 1) {
    return durationOffset + 8 <= boxEnd
      ? readUint64(view, durationOffset)
      : null;
  }
  return durationOffset + 4 <= boxEnd ? view.getUint32(durationOffset) : null;
}

function isUsableMovieDuration(
  timescale: number,
  duration: number | null,
  version: number,
): duration is number {
  if (!timescale || duration === null || duration <= 0) return false;
  return version !== 0 || duration !== 0xffff_ffff;
}

function parseTrackDimensions(
  view: DataView,
  box: IsoBox,
  start: number,
...
function parseHandlerType(
  view: DataView,
  box: IsoBox,
  start: number,
): string | null {
  const contentStart = start + box.headerSize;
  if (contentStart + 24 > box.end || view.getUint8(contentStart) !== 0)
    return null;
...
function parseMediaHandlerType(
  view: DataView,
  media: IsoBox,
  mediaStart: number,
  budget: IsoBoxBudget,
  ...
  let childOffset = mediaStart + media.headerSize;
  while (childOffset < media.end) {
    const child = readNestedIsoBox(view, childOffset, media.end, budget);
    if (!child) return null;
    if (child.type === "hdlr") {
      if (sawHandler) return null;
      sawHandler = true;
      handlerType = parseHandlerType(view, child, childOffset);
    }
    childOffset = child.end;
  }
  if (childOffset !== media.end) return null;
  return handlerType;
}

function isCompleteVideoTrack(
  childOffset: number,
  trackEnd: number,
  handlerType: string | null,
): boolean {
  return childOffset === trackEnd && handlerType === "vide";
}

type VideoTrackScan = {
  dimensions: { width: number; height: number } | null;
  handlerType: string | null;
  sawTrackHeader: boolean;
  sawMedia: boolean;
};

function recordVideoTrackChild(
  view: DataView,
  child: IsoBox,
  childOffset: number,
  budget: IsoBoxBudget,
  ...
  if (child.type === "tkhd") {
    if (scan.sawTrackHeader) return false;
    scan.sawTrackHeader = true;
    scan.dimensions = parseTrackDimensions(view, child, childOffset);
  }
  if (child.type === "mdia") {
    if (scan.sawMedia) return false;
    scan.sawMedia = true;
    scan.handlerType = parseMediaHandlerType(view, child, childOffset, budget);
    if (scan.handlerType === null) return false;
  }
  return true;
}
...
function parseVideoTrackDimensions(
  ...
  const scan: VideoTrackScan = {
    dimensions: null,
    handlerType: null,
    sawTrackHeader: false,
    sawMedia: false,
  };
  ...
  while (childOffset < track.end) {
    ...
    if (!child) return null;
    if (!recordVideoTrackChild(view, child, childOffset, budget, scan))
      return null;
    childOffset = child.end;
  }
  if (!isCompleteVideoTrack(childOffset, track.end, scan.handlerType))
    return null;
  return scan.dimensions;
}

function largerVideoDimensions(
...
function parseIsoBmffMovieBox(
  buffer: ArrayBuffer,
): VideoContainerMetadata | null {
  const view = new DataView(buffer);
  const budget = { remaining: ISO_BMFF_MAX_NESTED_BOXES };
  const movieBox = readNestedIsoBox(view, 0, view.byteLength, budget);
  if (!movieBox || movieBox.type !== "moov" || movieBox.end !== view.byteLength)
    return null;
  ...
  while (childOffset < movieBox.end) {
    ...
    if (child.type === "mvhd") {
      ...
      dimensions = largerVideoDimensions(
        dimensions,
        parseVideoTrackDimensions(view, child, childOffset, budget),
      );
    }
    childOffset = child.end;
  }
  if (childOffset !== movieBox.end || durationSeconds === null || !dimensions)
    return null;
  return {
...
async function readBlobArrayBuffer(blob: Blob): Promise<ArrayBuffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.addEventListener(
      ...
      () =>
        reject(
          reader.error ?? new Error("The selected video could not be read"),
        ),
      { once: true },
    );
...
function supportsIsoBmffMetadataFallback(file: File): boolean {
  if (!Number.isSafeInteger(file.size)) return false;
  if (file.size <= 0 || file.size > ISO_BMFF_MAX_FILE_BYTES) return false;
  if (/\.(?:mp4|mov)$/i.test(file.name)) return true;
  return file.type === "video/mp4" || file.type === "video/quicktime";
}

async function readIsoTopLevelBox(
  file: File,
  offset: number,
): Promise<IsoTopLevelBox | null> {
  const headerLength = Math.min(ISO_BMFF_HEADER_BYTES, file.size - offset);
  const headerBuffer = await readBlobArrayBuffer(
    file.slice(offset, offset + headerLength),
  );
  ...
  if (size32 === 1) {
    ...
    if (extendedSize === null) return null;
    return { type, headerSize: 16, size: extendedSize, headerView };
  }
  const size = size32 === 0 ? file.size - offset : size32;
  return { type, headerSize: 8, size, headerView };
}

function hasPrintableMajorBrand(headerView: DataView): boolean {
  return [8, 9, 10, 11].every((index) => {
    const byte = headerView.getUint8(index);
    return byte >= 0x20 && byte <= 0x7e;
  });
}

function recordIsoTopLevelBox(
  scan: IsoTopLevelScan,
  box: IsoTopLevelBox,
  offset: number,
): boolean {
  if (box.type === "ftyp") {
    if (scan.sawFileType || scan.movieRange) return false;
    if (box.headerSize !== 8 || box.size < 16) return false;
    if (!hasPrintableMajorBrand(box.headerView)) return false;
    scan.sawFileType = true;
  }
  if (box.type === "moov") {
    if (!scan.sawFileType || scan.movieRange) return false;
    if (box.size > ISO_BMFF_MAX_MOOV_BYTES) return false;
    scan.movieRange = { start: offset, size: box.size };
  }
  return true;
}

async function scanIsoTopLevelBoxes(
  file: File,
  signal?: AbortSignal,
): Promise<IsoTopLevelScan | null> {
  const scan: IsoTopLevelScan = { sawFileType: false, movieRange: null };
  let offset = 0;
  let boxCount = 0;
  while (offset + 8 <= file.size && boxCount < ISO_BMFF_MAX_TOP_LEVEL_BOXES) {
    if (signal?.aborted) return null;
    const box = await readIsoTopLevelBox(file, offset);
    if (signal?.aborted || !box) return null;
    if (!Number.isSafeInteger(box.size)) return null;
    if (box.size < box.headerSize || box.size > file.size - offset) return null;
    if (!recordIsoTopLevelBox(scan, box, offset)) return null;
    offset += box.size;
    boxCount += 1;
  }
  if (offset !== file.size || !scan.sawFileType || !scan.movieRange)
    return null;
  return scan;
}
...
async function readIsoBmffMetadata(
  ...
  if (!supportsIsoBmffMetadataFallback(file)) return null;
  const scan = await scanIsoTopLevelBoxes(file, signal);
  if (!scan || signal?.aborted) return null;
  const movieRange = scan.movieRange;
  if (!movieRange) return null;
  const movieBuffer = await readBlobArrayBuffer(
    file.slice(movieRange.start, movieRange.start + movieRange.size),
  );
  if (signal?.aborted) return null;
...
export const validateVideoAspectRatio = (
  ...
  return new Promise((resolve) => {
    ...
    const finish = (thumbnailUrl: string | null) => {
      if (resolved) return;
      resolved = true;
      const nativeDimensionsAreUsable =
    ...
    const beginFrameCaptureWhenMetadataIsUsable = () => {
      ...
      const targetTime =
        video.duration > 1 ? Math.min(0.5, video.duration - 0.1) : 0;
    ...
    metadataTimeout = window.setTimeout(() => {
      metadataTimeout = undefined;
      void readIsoBmffMetadata(file, signal)
        .then((metadata) => {
          if (resolved) return;
