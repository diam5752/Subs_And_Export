...
const OBSERVABILITY_API_BASE = process.env.NEXT_PUBLIC_API_URL
...
function presenceId(): string {
    ...
    runtimePresenceId = globalThis.crypto?.randomUUID?.()
...
function safeError(error: unknown): { name: SafeErrorName; statusCode?: number } | null {
    if (typeof error !== 'object' || error === null) return { name: 'unknown_error' };
    const value = error as { code?: unknown; status?: unknown };
    const code = typeof value.code === 'string' ? value.code : '';
    const statusCode = typeof value.status === 'number' ? value.status : undefined;
    if (code === 'upload_cancelled') return null;
    if (ERROR_CODE_BUCKETS[code]) return { name: ERROR_CODE_BUCKETS[code], statusCode };
...
export function reportBrowserError(name: 'window_error' | 'unhandled_rejection'): void {
    void send({
        kind: 'frontend_error',
        name,
        route: currentRoute(),
...
export function exportFormatBucket(resolution: string): ExportFormat {
    ...
    if (resolution === 'srt' || resolution === 'vtt' || resolution === 'txt') return resolution;
    return 'other';
}
...
export async function fetchObservabilitySnapshot(): Promise<ObservabilitySnapshot> {
    ...
    if (!response.ok) throw new Error(`observability_snapshot_${response.status}`);
