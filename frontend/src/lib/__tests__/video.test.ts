...
function makeTrack({
    ...
    return makeIsoBox('trak', concatenateBytes(
        makeIsoBox('tkhd', makeTrackHeader({ version, width, height, quarterTurn })),
        makeIsoBox('mdia', makeIsoBox('hdlr', handler)),
    ));
}

function makeFileFromBytes(bytes: Uint8Array, name = 'container-metadata.mp4', type = 'video/mp4'): File {
    return {
        name,
        type,
        size: bytes.length,
...
function makeIsoBmffFile({
    ...
    const moviePayload = concatenateBytes(
        ...
        ...(duplicateMovieHeader ? [makeIsoBox('mvhd', makeMovieHeader(version))] : []),
    ...
    const bytes = concatenateBytes(
        ...(omitFileType ? [] : [makeIsoBox('ftyp', new Uint8Array([
            0x69, 0x73, 0x6f, 0x6d,
            0x00, 0x00, 0x02, 0x00,
        ]))]),
...
describe('video utils', () => {
    ...
    describe('validateVideoAspectRatio', () => {
        ...
        it.each([
            ...
            ['audio-first rotated video track', { audioFirst: true, quarterTurn: true }],
            ['extended movie box after media data', { extendedMovieBox: true, movieAfterMedia: true }],
        ])('parses %s without trusting the native media element', async (_name, options) => {
            await expect(validateUsingContainerMetadata(makeIsoBmffFile(options))).resolves.toMatchObject({
                width: 1080,
                height: 1920,
                durationSeconds: 8.633,
                aspectWarning: false,
        ...
        it('does not read an oversized ISO BMFF movie box into memory', async () => {
            ...
            const fileTypeHeader = makeIsoBox('ftyp', new Uint8Array([
                0x69, 0x73, 0x6f, 0x6d,
                0x00, 0x00, 0x02, 0x00,
            ]));
        ...
        it('cancels stale metadata work and revokes the object URL', async () => {
            ...
            const promise = validateVideoAspectRatio(makeIsoBmffFile(), controller.signal);
