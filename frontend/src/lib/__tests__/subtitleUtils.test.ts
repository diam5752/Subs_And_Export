...
describe('resegmentCues', () => {
    ...
    it('caches segmentation results based on cue reference', () => {
        const cue: Cue = {
            start: 0,
            end: 1,
            text: 'Cache Test',
            words: [{ start: 0, end: 1, text: 'Cache Test' }]
        };

        // First call
        const result1 = resegmentCues([cue], 2, 100);

        // Second call - same cue reference
        const result2 = resegmentCues([cue], 2, 100);

        // Result content should be identical
        expect(result1).toEqual(result2);

        // Importantly, result object REFERENCE should be identical (if caching works)
        ...
        // Fourth call - same params as third
        const result4 = resegmentCues([cue], 1, 200);
        expect(result4[0]).toBe(result3[0]);
    });
});
