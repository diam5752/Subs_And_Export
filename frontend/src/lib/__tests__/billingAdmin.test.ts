...
describe('billing admin presentation helpers', () => {
    it('derives deterministic current second and minute values', () => {
        const dateNow = jest.spyOn(Date, 'now').mockReturnValue(
            Date.parse('2026-07-26T07:08:09.987Z'),
        );

        expect(currentEpochSeconds())
            .toBe(Date.parse('2026-07-26T07:08:09Z') / 1000);
        expect(currentMinuteEpochSeconds())
            .toBe(Date.parse('2026-07-26T07:08:00Z') / 1000);
    ...
    it.each([
        ...
        'ΠΑΡ/1',
        'ΣΕΙΡΑ_2.0',
    ])('accepts an AADE-compatible series: %s', (series) => {
    ...
    it.each([
        ...
        'SERIES 1',
        'A#1',
    ])('rejects an unsupported AADE series: %s', (series) => {
    ...
    it.each([
        ...
        '400014466064287',
        '9223372036854775807',
    ])('accepts a canonical xs:long-compatible MARK: %s', (mark) => {
    ...
    it.each([
        ...
        'MARK-1',
    ])('rejects a non-canonical MARK: %s', (mark) => {
    ...
    it('round-trips winter and summer Europe/Athens date-time values', () => {
        ...
        expect(parseAthensDateTime('2026-01-15T14:30'))
            .toBe(Date.parse('2026-01-15T12:30:00Z') / 1000);
        expect(parseAthensDateTime('2026-07-15T14:30'))
            .toBe(Date.parse('2026-07-15T11:30:00Z') / 1000);
        expect(toAthensDateTimeValue(
