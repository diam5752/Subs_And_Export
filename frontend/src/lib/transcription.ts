...
export function resolveConfiguredTranscription(
  ...
  const provider = RUNTIME_PROVIDERS.has(
    normalizedProvider as RuntimeTranscriptionProvider,
  )
    ? (normalizedProvider as RuntimeTranscriptionProvider)
    : "mock";
  const requestedMode: TranscriptionTier =
    (modeValue ?? "").trim().toLowerCase() === "pro" ? "pro" : "standard";
  ...
  const mode: TranscriptionTier =
    provider === "elevenlabs" ? "pro" : requestedMode;
