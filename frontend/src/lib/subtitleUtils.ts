...
export { findCueAtTime, findCueIndexAtTime } from "./subtitleSearch";
...
function getEffectiveMaxChars(fontSizePercent: number): number {
  ...
  const fontSizePx = Math.round(
    DEFAULT_SUB_FONT_SIZE * (fontSizePercent / 100),
  );
...
function createTextMeasurer(fontSizePercent: number): TextMeasurer | null {
  ...
  const fontSizePx = Math.max(
    1,
    Math.round(DEFAULT_SUB_FONT_SIZE * (fontSizePercent / 100)),
  );
  ...
  return {
    ...
    maxLineWidth: BASE_SAFE_WIDTH * 0.9,
  };
}
...
function wrapItemsBalanced<T>(
  ...
  const bestLayout = (
    ...
    for (let endIndex = startIndex; endIndex < items.length; endIndex += 1) {
      ...
      runningLength =
        endIndex === startIndex ? text.length : runningLength + 1 + text.length;
...
function scoreWrappedTextLines(
  ...
  const fillRatio =
    lengths.reduce((sum, length) => sum + Math.min(length, safeMaxChars), 0) /
    (safeMaxLines * safeMaxChars);
  const imbalance =
    lengths.length > 1
      ? (Math.max(...lengths) - Math.min(...lengths)) / safeMaxChars
      : 0;
  const unusedLinePenalty =
    remainingItems > 0 && wrappedLines.length < safeMaxLines
...
function lineWidth(texts: string[], measurer: TextMeasurer): number {
  if (texts.length === 0) return 0;
  return (
    texts.reduce((sum, text) => sum + measurer.measureText(text), 0) +
    Math.max(0, texts.length - 1) * measurer.spaceWidth
  );
}
...
function scoreWrappedWidthLines(
  ...
  const widths = wrappedLines.map((line) =>
    lineWidth(
      line.map((word) => word.text),
      measurer,
    ),
  );
  const safeMaxLines = Math.max(1, maxLines);
  ...
  const fillRatio =
    widths.reduce(
      (sum, width) => sum + Math.min(width, measurer.maxLineWidth),
      0,
    ) /
    (safeMaxLines * measurer.maxLineWidth);
  const imbalance =
    widths.length > 1
      ? (Math.max(...widths) - Math.min(...widths)) / measurer.maxLineWidth
      : 0;
  const unusedLinePenalty =
    remainingItems > 0 && wrappedLines.length < safeMaxLines
      ? ((safeMaxLines - wrappedLines.length) / safeMaxLines) * 0.25
...
function expandPhraseTiming(
  ...
  for (let i = 0; i < parts.length; i += 1) {
    ...
    const fraction =
      safeTotalChars > 0 ? charCount / safeTotalChars : 1 / parts.length;
...
function interpolateWordsFromCueText(cue: Cue): TranscriptionWordTiming[] {
  ...
  for (let i = 0; i < cueWords.length; i += 1) {
    ...
    const fraction =
      safeTotalChars > 0 ? wordChars / safeTotalChars : 1 / cueWords.length;
...
function chunkTimedWords(
  ...
  const bestChunking = (
    ...
    for (let endIndex = startIndex; endIndex < words.length; endIndex += 1) {
      ...
      const wrapped = wrapItemsBalanced(
        candidateTexts,
        (text) => text,
        maxChars,
      );
      ...
      const chunkScore = scoreWrappedTextLines(
        wrapped,
        maxChars,
        maxLines,
        words.length - endIndex - 1,
      );
...
function chunkTimedWordsByWidth(
  ...
  const bestChunking = (
    ...
    for (let endIndex = startIndex; endIndex < words.length; endIndex += 1) {
      ...
      const wrapped = wrapItemsByWidth(
        candidate,
        (word) => word.text,
        measurer,
      );
      ...
      const chunkScore = scoreWrappedWidthLines(
        wrapped,
        measurer,
        maxLines,
        words.length - endIndex - 1,
