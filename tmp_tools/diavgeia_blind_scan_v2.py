...
def corrected_score(candidate: base.Candidate, records: list[base.Record]) -> base.Candidate:
    candidate = base.score(candidate, records)
    cluster = [records[index] for index in candidate.indexes]

    # Reject the old shortcut: never sum every payment in a similarity cluster
    # against the largest award/commitment. Only an explicit ADA/ADAM-linked
    # connected component may produce a financial-consistency family.
