...
QUALITY_RUNNER = REPO_ROOT / ".codex" / "scripts" / "quality_runner.py"
...
def quality_command(arguments: Sequence[str]) -> str:
    if len(arguments) > 1:
        raise ValueError("Usage: run_isolated_quality_gate.py [check:<gate>]")
    command = arguments[0] if arguments else "check:all"
    if not command.startswith("check:"):
        raise ValueError("Quality command must start with `check:`")
    return command


def main(arguments: Sequence[str] | None = None) -> int:
    try:
        command = quality_command(sys.argv[1:] if arguments is None else arguments)
    ...
    parsed = urlparse(admin_url)
    database_name = f"gsp_ci_{os.getpid()}_{secrets.token_hex(4)}"
    ...
    with connection:
        ...
        environment.setdefault("GSP_JAVA_TEST_ADMIN_DATABASE_URL", admin_url)

        try:
            completed = subprocess.run(
                [sys.executable, str(QUALITY_RUNNER), command],
                cwd=REPO_ROOT,
                env=environment,
                check=False,
            )
